...
def analyze_audio(file_path, freq_min=0, freq_max=5000, time_interval=0.05, nfft=441000, csv_output="output.csv"):
    ...
    # 주요 눈금 및 세부 눈금 설정
    plt.gca().yaxis.set_major_locator(plt.MultipleLocator(10))  # y축 주요 눈금 간격 설정 (예: 100Hz 간격)
    plt.gca().yaxis.set_minor_locator(plt.MultipleLocator(1))  # y축 세부 눈금 간격 설정 (예: 20Hz 간격)

    # 격자 추가 (세부 눈금 포함)
    plt.grid(which='both', linestyle='--', linewidth=0.5, alpha=0.7)  # 세부 격자를 추가하여 빽빽하게 표시
    plt.tight_layout()
    plt.show()
...
def main():
    # 사용 예시
    for i in [1, 2, 5, 6, 7, 10, 21, 22, 25, 26, 27, 30]:
        file = f"경기북과학고등학교 {i}"
        file_path = f"audios/{file}.wav"
        output_path = f"datas/{file}.csv"
        analyze_audio(file_path, freq_min=7900, freq_max=8100, time_interval=0.05, csv_output=output_path)
